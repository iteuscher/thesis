...
In this chapter, an analytical framework for measuring document protection methods is presented consistent with approaches taken to evaluate authentication schemes in related domains~\cite{alaca_comparative_2021-1,alaca_comparative_2021,bonneau_quest_2012,borisov_sok_2021}. This framework provides a semi-structured evaluation methodology and benchmark for future document protection methods~\cite{bonneau_quest_2012}. An earlier version of this framework and its application to seven document encryption methods was published as Teuscher and Schooley~\cite{teuscher_document_2025}; this chapter extends that work to nine methods and adds additional design properties. We aim to answer the first research question: \emph{What usability, deployability, and security limitations exist in current document encryption methods?}
...
\begin{landscape}
\input{tables/uds-comparison}
\end{landscape}

\subsection{Password-based encryption}
\label{sec:eval-password}

One of the most common forms of document encryption today is password-based file-level encryption, which is built into tools like Microsoft Word\textregistered{} and Adobe Acrobat\textregistered{}. Typically, AES 256-bit encryption is used to protect the entire document with a single password (up to 15 characters) used to derive the encryption key.
